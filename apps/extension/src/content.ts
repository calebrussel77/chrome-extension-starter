...
// Font family constant using Inter
...
const handleTextSelection = async () => {
  ...
  if (text && text !== selectedText) {
    ...
    if (config && config.autoTranslate && config.openaiApiKey) {
      isTranslating = true;
      showLoadingPopup();
...
const loadInterFont = () => {
  if (!document.getElementById("ai-translator-font")) {
    ...
    fontLink.id = "ai-translator-font";
...
const showLoadingPopup = () => {
  ...
  loadingContainer.style.fontFamily = FONT_FAMILY;
  ...
  titleDiv.style.fontFamily = FONT_FAMILY;
  ...
  loadingText.style.fontFamily = FONT_FAMILY;
...
const showTranslation = (translatedText: string, originalText: string) => {
  ...
  originalDiv.style.fontFamily = FONT_FAMILY;
  ...
  translatedDiv.style.fontFamily = FONT_FAMILY;
...
const showError = (errorMessage: string) => {
  ...
  alertDiv.style.fontFamily = FONT_FAMILY;
  ...
  errorTitle.style.fontFamily = FONT_FAMILY;

  const errorText = document.createElement("div");
  errorText.textContent = errorMessage;
  errorText.style.color = "#ef4444";
  errorText.style.fontSize = "13px";
  errorText.style.fontFamily = FONT_FAMILY;
